...
import struct
# Define the RS485 settings
PORT = '/dev/ttyUSB0'  # Change this to your RS485 port
...
client = ModbusClient(method='rtu', port='/dev/ttyUSB0', baudrate=115200, stopbits=1, bytesize=8, parity='N')

print(client.connect())

def modbus_registers_to_float(registers):
    # Concatenate the two 16-bit registers into a 32-bit integer
    raw = (registers[0] << 16) | registers[1]

    # Unpack the 32-bit integer as a float using struct
...
# Example array of 16-bit registers (assuming Big Endian format)
registers_float = [0x4197, 0x36f3]  # Example registers

# Example array of 16-bit registers for double
registers_long = [0x3ff0, 0x0000, 0x0000, 0x0000] # Example registers


# Map Modbus addresses to keys for PostgreSQL table
modbus_to_key_map = {
...
# Function to read data from each register address
...
def insert_into_table(arr, value):
    try:
        conn = psycopg2.connect(
            dbname="lapistest",
            user="postgres",
            password="Pass4lapisclouddata",
            host="lapisclouddata.ctlyit22sgfp.us-east-2.rds.amazonaws.com",
            port=5432
        )
        data = arr[0]
        unit = arr[1]
...
# Main function to read data from multiple register addresses and insert into PostgreSQL table
def main():
    while True:
